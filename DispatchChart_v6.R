...
mylocalresults = 'C:/Users/01425453/Google Drive/Work/SANEDI DSM/R codes/Transport results/Charts'

resultsDir = mylocalresults
...
myyear <- 2015 # year we want to look at
...
#get the scenario we want to look at in the year we want to look at
FOUT = subset(FOUT,FOUT$Scenario == myscen & FOUT$Year == myyear )
FIN = subset(FIN,FIN$Scenario == myscen & FIN$Year == myyear)
  ...
  #EXTRACT FROM THE DATAFRAMES:
    ...
    #convert energy to power ****
      ...
      pwrData$GW <- pwrData$Value*277.78/(8760*pwrData$tsfr) # this is GW
...
# extend all data entries for processes into the full ts detail:
  ...
  pwr_s6 <- subset(pwrData,pwrData$tslen == 6)
  pwr_s6 <- pwr_s6[,((names(pwr_s6) %in% c('Process','TimeSlice','GW')))]#drop the original names column)]
  
  pwr_s2 <- subset(pwrData,pwrData$tslen != 6)
  ...
  colnames <- c('Process','TimeSlice.y','GW')
  ...
  names(tmp) <- c('Process','GW','TimeSlice') #basically renaming timeslice.y to timeslice
...
# Make chart of sector demand profiles (Shows res+com+ind) ******************
  ...
  elc_prcs$GW <- elc_prcs$Value*277.78/(8760*elc_prcs$tsfr) # this is GW
  ...
  p1 <- ggplot(plotdat,aes(x = Hour, y = GW, fill = Sector)) +geom_bar(stat = 'identity')+
    ggtitle(chart_title) + ylab('GW')+
...
# make charts of by sector and end use *************
  ...
  {#loop over each sector and make a chart for its dispatch
    ...
    p1 <- ggplot(plotdat,aes(x = Hour, y = GW, group = EndUse, colour = EndUse)) +geom_line(size = 1.2)+
      ggtitle(chart_title) + ylab('GW')+
